...
GidFile::Result GidFile::write(QString filename, QByteArray data)
{
    ...
    if (forig.exists()) {
        ...
        if (fbackup.exists()) {
            ...
            if (!fbackup.remove()) {
                ret.success = false;
                ret.errorString = "Failed to remove previous backup file: " + fbackup.errorString();
                return ret;
            }
        }
        ...
        if (!forig.rename(fbackup.fileName())) {
            ret.success = false;
            ret.errorString = "Failed to backup original file: " + forig.errorString();
            return ret;
        }
    }
    ...
    if (!ftemp.rename(filename)) {
        ...
        if (!ftemp.rename(filename)) {
            ret.success = false;
            ret.errorString = "Failed to replace original file: " + ftemp.errorString();
            return ret;
        }
    }
